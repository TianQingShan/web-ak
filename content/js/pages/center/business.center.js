var _vue = new Vue({
    ...
    methods: {
        ...
        'loadBuyData': function () {
            this.tabControl[0].pageIndex = 1;
            this.tabControl[0].isLoading = true;
			window.scrollBottom = this.windowScroll0;
            APP.GLOBAL.ajax({
                url: APP.CONFIG.BASE_URL+'My_EP_BuyRecords',
                data: {
                    'p': this.tabControl[0].pageIndex,
                    'pageSize': this.tabControl[0].pageSize
                },
                success: function (result) {
                    if (result.Error) {
                        APP.GLOBAL.toastMsg(result.Msg);
                    ...
                    _vue.tabControl[0].pageIndex++;
                    _vue.tabControl[0].list = result.Data.List;
                    _vue.tabControl[0].isLoading = false;
		...
		'loadMore0': function () {
            APP.GLOBAL.ajax({
                ...
                success: function (result) {
                    ...
                    _vue.tabControl[0].pageIndex++;
                    _vue.tabControl[0].list = _vue.tabControl[0].list.concat(result.Data.List);
                    _vue.tabControl[0].isLoadMore0 = false;
					
                    if (result.Data.List.length < _vue.tabControl[0].pageSize) {
                        _vue.tabControl[0].isLoadComplete0 = true;
                    }
                }
            });
        },
		...
		'windowScroll0': function () {
            if (!this.tabControl[0].isLoadMore0 && !this.tabControl[0].isLoadComplete0) {
                this.tabControl[0].isLoadMore0 = true;
                this.loadMore0();
            }
        },
