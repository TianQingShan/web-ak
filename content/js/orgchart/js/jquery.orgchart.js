...
(function (factory) {
    ...
    function createNode(nodeData, level, opts) {
        var isEmpty = nodeData.id == "0" ? true : false;
        var dtd = $.Deferred();
        ...
        var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + '>')
          .addClass('node ' + (nodeData.className || '') + (level >= opts.depth ? ' slide-up' : ''))
                .append(isEmpty ? '<div class="title">' + nodeData[opts.nodeTitle] + '</div>' : '<div class="title">' + nodeData[opts.nodeTitle] + '</div>')
            .append(typeof opts.nodeContent !== 'undefined' ? '<div ' + (!isEmpty ? 'onclick="_vue.loadNextLayer(' + nodeData.flowNumber + ')"' : 'onclick="_vue.newReport(' + nodeData.pid + ', ' + nodeData.position + ')"') + ' class="content">' + (nodeData[opts.nodeContent] || '') + '</div>' : '');
        // append 4 direction arrows
        var flags = nodeData.relationship || '';
        if (sysName == "ios") {
            if (Number(flags.substr(0, 1))) {
                $nodeDiv.append('<i class="edge verticalEdge topEdge fa fa-chevron-down"></i>');
            }
            if (Number(flags.substr(1, 1))) {
                $nodeDiv.append('<i class="edge horizontalEdge rightEdge fa fa-chevron-left"></i>' +
                  '<i class="edge horizontalEdge leftEdge fa fa-chevron-right"></i>');
            }
        ...
        $nodeDiv.on('click', '.topEdge', function (event) {
            ...
            if (parentState.exist) {
                ...
                if (startLoading($that, $node, opts)) {
                    // load new nodes
                    $.ajax({ 'url': opts.ajaxURL.parent + '&id=' + nodeId, 'dataType': 'json', 'method': 'post' })
                    .done(function (data) {
        ...
        $nodeDiv.on('click', '.bottomEdge', function (event) {
            ...
            if (childrenState.exist) {
                ...
                var nodeId = $that.parent()[0].id;
                if (startLoading($that, $node, opts)) {
                    $.ajax({ 'url': opts.ajaxURL.children + '&id=' + nodeId, 'dataType': 'json', 'method': 'post' })
                    .done(function (data, textStatus, jqXHR) {
                        if ($node.closest('.orgchart').data('inAjax')) {
        ...
        $nodeDiv.on('click', '.leftEdge, .rightEdge', function (event) {
            ...
            if (siblingsState.exist) {
                ...
                if (startLoading($that, $node, opts)) {
                    $.ajax({ 'url': url + '&id=' + nodeId, 'dataType': 'json', 'method': 'post' })
                    .done(function (data, textStatus, jqXHR) {
                        if ($node.closest('.orgchart').data('inAjax')) {
